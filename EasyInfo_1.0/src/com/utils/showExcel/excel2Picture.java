...
import com.utils.showExcel.domain.Grid;
import com.utils.showExcel.domain.UserCell;

//import java.awt.*;
/**
 * width:pix=getColumnWidthInPixels*1.15 height:pix=getHeightInPoints*96/72
 * 
 * 本示例用来读取Excel报表文件，并力图将报表无差别转化成PNG图片 使用POI读取Excel各项数据 使用JAVA 2D绘制PNG TODO
 * 本示例基本实现了常见Excel的所有样式输出，但Office2007以后的版本添加了条件样式功能，因为POI的API无法读取条件样式，所以无法实现
 * 今后可以通过关键字标注等方法，在Excel中需要加入条件样式的单元格用注解标明，使用JAVA计算得出样式再绘制出来
 */
public class excel2Picture {

	public String createPng(String toFile) throws Exception {
		Workbook wb = WorkbookFactory.create(new File(toFile));
		Sheet sheet = wb.getSheetAt(0);
		List<CellRangeAddress> rangeAddress = sheet.getMergedRegions(); // 获取整个sheet中合并单元格组合的集合
		int colCount = 0;
		for (int i = 0; i < sheet.getLastRowNum(); i++) {
			if (sheet.getRow(i).getLastCellNum() > colCount)
				colCount = sheet.getRow(i).getLastCellNum();
		}
		// 给定两个初始值，标志出导出区域，两个行列组合的单元格
		int[] fromIndex = { 0, 0 };
		int[] toIndex = { sheet.getLastRowNum(), colCount };// 后期传参解决，前者为行数，后者为列数
		...
		if (fromIndex[0] > rowSum || fromIndex[0] > toIndex[0]
				|| toIndex[0] > rowSum) {
			// throw new Exception("the rowIndex of the area is wrong!");
		}
		if (fromIndex[1] > colSum || fromIndex[1] > toIndex[1]
				|| toIndex[1] > colSum) {
		...
		for (int i = 0; i < rowSize; i++) {
			...
			for (int j = 0; j < colSize; j++) {
				...
				ifShow = ifShow
						&& !(sheet.isColumnHidden(j) || sheet.getRow(i)
								.getZeroHeight()); // 其次行列不可以隐藏
				...
				colPixPos[j + 1] = (int) (widthPix * 1.343 + colPixPos[j]);// 默认1.15
			...
			float heightPoint = !ifShow ? 0 : sheet.getRow(i)
					.getHeightInPoints(); // 如果该单元格是隐藏的，则置高度为0
			imageHeight += heightPoint;
			rowPixPos[i + 1] = (int) (heightPoint * 1.295) + rowPixPos[i];// 96
																			// /
																			// 72
																			// -94
																			// /
		...
		for (int i = 0; i < rowSize; i++) {
			for (int j = 0; j < colSize; j++) {
				...
				if (isInMergedStatus[0] == 0 && isInMergedStatus[1] == 0) {
					...
					int lastRowPos = isInMergedStatus[0] > rowSize - 1 ? rowSize - 1
							: isInMergedStatus[0];
					int lastColPos = isInMergedStatus[1] > colSize - 1 ? colSize - 1
							: isInMergedStatus[1];
				...
				// grid.setBgColor(cells[i][j].getCell().getCellStyle()
				// .getFillForegroundColorColor());

				// 设置字体
				org.apache.poi.ss.usermodel.Font font = wb.getFontAt(cs
						.getFontIndex());
				grid.setFont(font);
				...
				case HSSFCell.CELL_TYPE_NUMERIC:
					strCell = String.valueOf(cells[i][j].getCell()
							.getNumericCellValue());
					break;
				...
				if (cells[i][j].getCell().getCellStyle().getDataFormatString()
						.contains("0.00%")) {
					try {
						double dbCell = Double.valueOf(strCell);
						strCell = new DecimalFormat("#.00")
								.format(dbCell * 100) + "%";
					} catch (NumberFormatException e) {
					}
				}

				grid.setText(strCell.matches("\\w*\\.0") ? strCell.substring(0,
						strCell.length() - 2) : strCell);
		...
		// 平滑字体
		// g2d.setRenderingHint(SunHints.KEY_ANTIALIASING,
		// SunHints.VALUE_ANTIALIAS_OFF);
		// g2d.setRenderingHint(SunHints.KEY_TEXT_ANTIALIASING,
		// SunHints.VALUE_TEXT_ANTIALIAS_DEFAULT);
		// g2d.setRenderingHint(SunHints.KEY_STROKE_CONTROL,
		// SunHints.VALUE_STROKE_DEFAULT);
		// g2d.setRenderingHint(SunHints.KEY_TEXT_ANTIALIAS_LCD_CONTRAST, 140);
		// g2d.setRenderingHint(SunHints.KEY_FRACTIONALMETRICS,
		// SunHints.VALUE_FRACTIONALMETRICS_OFF);
		// g2d.setRenderingHint(SunHints.KEY_RENDERING,
		...
		for (Grid g : grids) {
			...
			g2d.drawString(
					...
					g.getY() + (g.getHeight() - font.getSize()) / 2
							+ font.getSize());
		}
		...
		String rootAddress = tempFile.getPath();
		rootAddress = rootAddress.substring(0, rootAddress.indexOf(fileName));
		ImageIO.write(image, "png", new File(rootAddress + fileName + ".png"));

		System.out
				.println("生成png成功" + "地址为：" + rootAddress + fileName + ".png");
		return rootAddress + fileName + ".png";
	}
